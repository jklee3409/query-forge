...
- Local retrieval now uses a cached BM25 + dense embedding ranker (`common/local_retriever.py`) for retrieval eval, answer eval, memory lookup, and gating utility scoring; `sentence-transformers` real embeddings are used when installed, otherwise BM25 + hash embedding fallback remains available.
