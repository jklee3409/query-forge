...
- `db-ann` memory lookup uses `memory_entries.query_embedding` filtered by `metadata.embedding_model` plus experiment/snapshot identity, while the online hash path remains isolated outside this runtime.
...
- Selective rewrite prompt loading is language-aware: English evaluation queries use `configs/prompts/rewrite/selective_rewrite_en_v1.md`, while Korean/code-mixed queries prefer `selective_rewrite_v2.md` with automatic fallback to `selective_rewrite_v1.md`.
