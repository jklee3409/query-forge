...
- Chat readiness APIs (`/api/chat/readiness`, `/api/admin/chat/readiness`) expose bounded per-domain health for active config, selected gating snapshot, source gating run, domain/strategy/preset mismatch, chunk embeddings, memory/query counts, prompt binding, retrieval tuple, and rewrite-blocking reasons.
