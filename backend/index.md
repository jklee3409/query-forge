...
- Corpus admin exposes anchor normalization dry-run/review APIs under `/api/admin/corpus/anchors/normalization-runs`; dry-runs materialize all candidates first, candidate-level `approve`/`skip` review decisions are saved in bulk, and run approval applies only approved safe updates to `corpus_glossary_terms.canonical_form` / `normalized_form`.
