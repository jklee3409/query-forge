...
- Live `/api/chat/ask` now completes with a real LLM answer stage: `RagService.ask` uses `ChatAnswerService` to generate the final response from the rewritten query plus retrieved context, persists `answerModel` / cited document IDs / cited chunk IDs, and keeps the existing synthetic-memory rewrite flow ahead of answer generation.
- Live chat rewrite/answer model resolution now falls back to the repo-root `.env` through `RuntimeEnvService`, so local `bootRun` sessions can use configured Gemini/OpenAI credentials even when the shell process itself was not started with those environment variables.
- Live chat dense query embeddings are spawned through `DenseEmbeddingService`; on Windows the embedded Python `-c` script now uses quote-safe single-quoted literals so `/api/chat/ask` does not lose `utf-8`/model config strings during native command-line parsing.
...
- Live chat can opt into a rule-based `QueryStrategyRouter` via `chat_runtime_config.metadata.routerEnabled`; the router chooses RAW_ONLY, SYNTHETIC_SELECTIVE_REWRITE, or ANCHOR_AWARE_REWRITE per request and records the decision in existing metadata logs without new tables.
- Live chat can opt into intra-domain agentic multi-query retrieval via `chat_runtime_config.metadata_json.agenticMultiQueryEnabled`; the path keeps `domainId` mandatory, generates domain-scoped subqueries, runs each subquery through the existing router/rewrite/retrieval flow sequentially, and records plan/trace metadata without new tables.
- Chat runtime config API now exposes `routerEnabled` as an explicit request/response field while continuing to persist it in `metadata_json.routerEnabled`; omitted metadata on updates preserves existing metadata.
- Live chat Gemini answer generation now retries one `503 ServiceUnavailable` response once inside `ChatAnswerService`; if the retry also fails, the RAG API returns a 503 ProblemDetail with `errorCode=GEMINI_SERVICE_UNAVAILABLE` and dedicated user-facing retry/failure copy.
- The backend-served React bundle includes Chat Settings UI polish for English runtime option labels, concise generation strategy labels, highlighted accepted-count badges, `full gating` preset copy, and compact readiness/status badge spacing without changing API contracts.
- Backend-served Admin React bundle includes the compact Chat Settings controls and scrollable sidebar presentation for domain workspaces.
