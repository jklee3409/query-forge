...
## Recent Update
- `/admin/synthetic-queries` now makes the selected synthetic generation strategy and chunk sampling option more prominent. Strategy A/B/C/D/E source selection is scoped to the five Spring reference sources, F/G is scoped to `docs-python-org-ko-3-14`, "전체 허용 소스" fans out only across that scoped list, and `arahansa-github-io-docs-spring` is hidden from the UI and excluded from request construction.
